...
public class DBnotification extends SQLiteOpenHelper {
    public DBnotification(Context context, String name, SQLiteDatabase.CursorFactory factory, int version) {
        super(context, name, factory, version);
    }

    public void onCreate(SQLiteDatabase db) {
        String sql = "create table if not exists NotiOnOff("
                + "id integer, "
                + "OnOff text);";
        db.execSQL(sql);
    }
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        String sql = "drop table if exists NotiOnOff";
        db.execSQL(sql);
        onCreate(db);
    }
    ...
    public void insert(int id, String OnOff) {
        ...
        db.insert("NotiOnOff", null, values);
        db.close();
    }
    ...
    public void update(int id, String OnOff) {
        ...
        //values.put("id", id);
        values.put("OnOff", OnOff); // 1 is On, 0 is OFF
        db.update("NotiOnOff", values, "id="+id, null);
        db.close();
    }
